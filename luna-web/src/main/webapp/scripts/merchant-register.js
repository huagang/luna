...
$(function() {
	...
	$('#province').change(function(){
		...
		if (province == 'ALL') {
			$("#province").addClass('warn-border').attr('data_accuracy', 'false');
		} else {
			$("#province").removeClass('warn-border').attr('data_accuracy', 'true');
		}
		var flag = $("#province").attr('data_accuracy') && $("#city").attr('data_accuracy');
		if (flag == 'false') {
	...
	$('#city').change(function() {
		...
		var $warn = $("#addressitem-warn");
		if (city == 'ALL') {
			$("#city").addClass('warn-border').attr('data_accuracy','false');
		} else {
			$("#city").removeClass('warn-border').attr('data_accuracy','true');
		}
		var flag = $("#province").attr('data_accuracy') && $("#city").attr('data_accuracy');
		if (flag == 'false') {
			$warn.css('display', 'block');
	...
	$("#btn-address").click(function() {
		...
		if ($address.length == 0) {
			$("#merchant_addr").addClass("warn-border").attr('data_accuracy', 'false');
			$warn_d.html("请输入详细地址").css('display', 'block');
		} else {
			$("#merchant_addr").removeClass("warn-border").attr('data_accuracy', 'true');
			$warn_d.css('display', 'none');
		}
		if ((flag == "false") || ($address.length == 0)) {
			$("#address-warn").html('请先填写地址').css('display','inline-block');
			return;
		} else {
			$("#address-warn").css('display', 'none');
			$("#address-container").css({"width" : "500px","height" : "250px"});
			// 获取输入地址
	...
	$("#btn-submit2").click(function(){
		...
		success : function(returndata) {
			...
			switch (status) {
			case '0':
				window.location.href = host + '/merchantRegist.do?method=success';// 成功后调整到页面
				break;
			case '1':
				window.location.href = host + '/error.jsp';
				break; // 校验失败
			case '2':
				$("#status-message").html("图片上传失败!").css('display', 'block');
				setTimeout(function() {
				...
				break; // 图片上传失败
			case '3':
				$("#merchant-name-warn").html('用户名重名').css("display", "block");
				$("#pop-overlay").css("display", "none");
				$("#pop-newbusiness").css("display", "none");
				break; // 用户重名
			default:
				$("#status-message").html(msg).css('display','block');
				setTimeout(function() {
					$("#status-message").css('display', 'none');
				}, 2000);
				break; // 创建失败
