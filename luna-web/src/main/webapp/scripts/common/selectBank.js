...
;
(function($) {
    function initBranchBank(options, isChanged) {
        ...
        if (!bankId || !cityId) {
            ...
            }).select2('val', isChanged ? null : $('#' + options.branchBankNameId).data('code'));
        }
    }
    /**
     * 清空data-code 内容
    ...
    $.extend({
        ...
        initSelectBank: function(options) {
            ...
            var $selBank = $("#" + options.bankId).select2({
                width: "off",
                data: objBankName,
                ...
                if ($('#' + options.bankNameId).data('code') != $("#" + options.bankId).val()) {
                    $('#' + options.bankNameId).attr('data-code', $("#" + options.bankId).val());
                    isChanged = true;
                }
                //初始化支行下拉
                initBranchBank(options, isChanged);
            });
            $selBank.select2('val', $('#' + options.bankNameId).data('code'));

            var $province = $("#" + options.provinceId).select2({
                // width: "off",
                placeholder: "请输入选择省份",
                allowClear: true,
                data: objProvinceNam,
                ...
                if ($('#' + options.provinceNameId).data('code') != $("#" + options.provinceId).val()) {
                    ...
                    isChanged = true;
                    // $("#" + options.cityId).select2('val', null);
                    // $("#" + options.branchBankId).select2('val', null);
                }
                /* Act on the event */
                ...
                if (provinceValue) {
                    $("#" + options.cityId).select2({
                        ...
                        /* Act on the event */
                        initBranchBank(options, isChanged);
                    }).select2('val', isChanged ? null : $('#' + options.cityNameId).data('code'));
                } else {
                    //没有省份初始化空的城市控件
            ...
            $province.select2('val', $('#' + options.provinceNameId).data('code'));
        }
    });
