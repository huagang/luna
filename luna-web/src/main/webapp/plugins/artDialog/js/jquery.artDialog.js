    ...
    */
    button: function () {
        ...
        for (; i < ags.length; i ++) {
            ...
            button.type = 'button';
            button.className = 'btn-default btn-lg ' + defaultBtn;
            ...
            if (val.focus) {
                ...
                this._focus = $(button).addClass(strongButton).removeClass(defaultBtn);
                this.focus();
            };
            
            button[_expando + 'callback'] = id;
            button.disabled = !!val.disabled;
            !!val.disabled ? $(button).addClass('btn-disabled').removeClass(defaultBtn) : $(button).removeClass('btn-disabled');
    ...
    */
    ...
    lock: function () {
        ...
        $div.bind('dblclick', function () {
            if(config.dbclickHide)that._click('cancel');
        });
    ...
    // 重置位置
    _reset: function () {
        if(!(this.config.fixed && $(this.dom.wrap[0]).css('top')===$(window).height()-$(this.dom.wrap[0]).height())){
