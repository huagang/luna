...
public class PoiApiBLImpl implements PoiApiBL {
	...
	private static String[] dbFields = null;// 所有接口字段对应的数据库字段名称
...
//	private static Map<String, String> fieldNamesLst = new LinkedHashMap<>();// (key：字段名称，value:字段显示名称)
	...
	public JSONObject getCtgrsByBizIdAndPoiId(String json) {
		...
		if (tree != null) {
			...
			for (String _id : poiIdLst) {
				...
				if (doc != null) {
					// ArrayList<Integer> tags = (ArrayList<Integer>)
					// doc.get("tags");
	...
	public JSONObject getPoiInfoById(String json) {
		...
		// 分享摘要--数据库中可能不存在该字段
		poi.put(this.getOuterVal("share_desc"), "");
		
		for (Entry<String, Object> entry : entrySet) {
			String key = entry.getKey();
			if (db2api_nm.containsKey(key) || "sub_tag".equals(key)) {
				// 一级类别--tags
				if ("tags".equals(key)) {
				...
				// 标签--type
				if ("type".equals(key)) {
					// ArrayList<Integer> types = (ArrayList<Integer>)
					// entry.getValue();
				...
				if ("lnglat".equals(key)) {
					...
					// ArrayList<Double> coordinates = (ArrayList<Double>)
					// lnglat.get("coordinates");
				...
				if ("zone_id".equals(key)) {
					...
					data.put(this.getOuterVal("zone_id"), zone_id);
				...
				// 详细地址
				if ("detail_address".equals(key)) {
					String detail_address = (String) entry.getValue();
					JSONObject data = poi.getJSONObject("address");
					if (data == null) {
						data = new JSONObject();
					}
					data.put(this.getOuterVal("detail_address"), detail_address);
					poi.put("address", data);
					continue;
				}
				// 全景标识
				if("panorama".equals(key)) {
					...
					data.put(this.getOuterVal("panorama"), panorama_id);
				...
				poi.put(this.getOuterVal(key), entry.getValue());
				// }
			}
		}
	...
	public void init() {
		...
		if(list != null && !list.isEmpty()) {
			String[] outerfields = new String[list.size()];
			String[] innerfields = new String[list.size()];
			for(int i = 0; i < list.size(); i++){
				...
				innerfields[i] = field_name;
				outerfields[i] = field_alias;
			}
			apiFields = outerfields;
			dbFields = innerfields;
		}
	}
...
//	}
	 ...
	 * @return
	 */
	private JSONArray getPoiInfoByTagsWithFields(Set<String> poiIdLst, String type, String[] tags, String[] fields,
			String lang) {
		MongoCollection<Document> poi_collection = null;
		if ("zh".equals(lang)) {
			poi_collection = mongoConnector.getDBCollection("poi_collection");
	 ...
	 * @return
	 */
	private JSONArray getPoiInfoBySubCtgrIdWithFields(List<String> poiIdLst, int sub_ctgr_id, String[] fields,
			String lang) {
		MongoCollection<Document> poi_collection = null;
		if ("zh".equals(lang)) {
			poi_collection = mongoConnector.getDBCollection("poi_collection");
				...
				if (resJson.containsKey(key) || "sub_tag".equals(key) || "panorama_type".equals(key)) { // "sub_tag"在mongo中但不在mysql field字段中，而panorama_type在mysql field字段但可能不在mongo中(前期未设计)
					...
					if (db2api_nm.containsKey(key) || "sub_tag".equals(key)) {
						...
						if ("lnglat".equals(key)) {
							...
							// (ArrayList<Double>) lnglat.get("coordinates");
							JSONArray coordinates = FastJsonUtil.parse2Array(lnglat.get("coordinates"));
							JSONObject data = new JSONObject();
						...
						// 详细地址
						if ("detail_address".equals(key)) {
							String detail_address = resJson.getString("detail_address");
							JSONObject data = result.getJSONObject("address");
							if (data == null) {
								data = new JSONObject();
							}
						...
						result.put(this.getOuterVal(key), resJson.get(key));
					}
				}
			}
		}
